...
#[template(resource = "/org/gtk_rs/CompositeTemplate6/window.ui")]
pub struct Window {
    pub number: Cell<i32>, // hold the state
}
...
impl Window {
    ...
    #[template_callback]
    fn handle_button4_clicked(&self, button:&CustomButton){ // swapped can use self
        // set the label to "Hello World" after the button has been clicked
...
impl ObjectSubclass for Window {
    ...
    fn class_init(klass: &mut Self::Class) {
        ...
        CustomButton::ensure_type();   // 取消属性定义的templateField时，需要添加算定义按钮注册
